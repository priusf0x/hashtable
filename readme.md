...
Для хранения данных был взят и немного изменен cache-friendly список(list), написанный в одной из предыдущих работ курса. Идея была в том, чтобы все данные обладали пространственной локальностью(однако это было не совсем корректным предположением, об этом будет изложено позжe), а память под всю таблицу выделялось в несколько аллокаций независимо от количества данных и корзин. С помощью предоставленного интерфейса пользователь может выбрать свою хеш функцию, добавлять слова, получать ключи. 
...
функции получения ключа в цикле. Время выполнения такого цикла записывалось в массив и процесс повторялся заново. Такие действия повторялись N раз, после чего значения усреднялись с учетом погрешности полученного значения. Тесты были достаточно большие, чтобы исключить время загрузки таблицы из памяти, запись в память и остальные накладные расходы не связанные с самим тестированием.   
...
Для оптимизации необходимо было сделать все горячие функции(хеш функции, функции работы со списком) inlined.
...
Было решено, что следующими оптимизациями будут использование Intel Intrinsics для подсчета хеш функции CRC32(точнее CRC32C) и написание своей версии strncmp с использованием векторных инструкций.
...
Как-то обойти процесс чтения без изменения структуры хеш таблицы не представляется возможным, однако и сам процесс чтения можно оптимизировать.  
